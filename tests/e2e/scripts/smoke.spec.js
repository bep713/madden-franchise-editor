...
test('basic test', async () => {
  ...
  const firstCell = await tableEditor.getTextAtCell(1, 1);
  ...
  await tableEditor.scrollToColumn();
  await new Promise((resolve) => {
    setTimeout(() => {
      resolve();
    }, 50000);
  });
  const sanFran = await tableEditor.getTextAtCell(1, 4);
  expect(sanFran).to.equal('San Francisco');

const {
  JSONEditor,
} = require("../../../../libs/vanilla-jsoneditor/jsoneditor.umd");
...
class BinaryBlobEditor {
    ...
    this.editor = new JSONEditor({
      ...
      props: {
        ...
        onRenderMenu: (items) => {
          return items.filter(
            (v) =>
              v.text !== "table" && v.text !== "tree" && v.type === "button",
          );
        },
      },
    });
  ...
  close() {
    ...
    if (editedData.text) {
      newData = JSON.stringify(JSON.parse(editedData.text));
    } else {
      newData = JSON.stringify(editedData.json);
    }

    this.tableEditorWrapper.selectedTableEditor.hot.setDataAtCell(
      hotRow,
      hotCol,
      newData,
    );

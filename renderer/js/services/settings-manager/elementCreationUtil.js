...
elementCreationUtil.createDropdownField = (field, currentValue) => {
  ...
  select.addEventListener("change", function () {
    wrapper.dispatchEvent(
      new CustomEvent("setting-change", { detail: select.value }),
    );
  });
...
elementCreationUtil.createCheckboxField = (field, currentValue) => {
  ...
  input.addEventListener("change", function () {
    wrapper.dispatchEvent(
      new CustomEvent("setting-change", { detail: input.checked }),
    );
  });
  ...
  directoryButton.addEventListener("click", async function () {
    ...
    if (directory && directory.length > 0) {
      ...
      textField.dispatchEvent(
        new CustomEvent("setting-change", { detail: input.value }),
      );
    }
  });
...
function inputEventListener(event) {
  ...
  wrapper.dispatchEvent(
    new CustomEvent("setting-change", { detail: event.srcElement.value }),
  );
}

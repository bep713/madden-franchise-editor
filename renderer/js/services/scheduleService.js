...
let scheduleYearChoices = [];
loadScheduleChoices().then((choices) => {
  scheduleYearChoices = choices;
});
...
async function addEventListeners() {
  ...
  replaceAllButton.addEventListener("click", async function () {
    closeModals();
    ...
    try {
      ...
      if (scheduleFile) {
        ...
        console.error(`Failed to load schedule file: ${selectedFile}`);
      }
    } catch (error) {
      console.error(`Error loading schedule file: ${selectedFile}`, error);
    } finally {
...
function attachGameElementsForWeek(weekNum) {
  scheduleService.file.schedule
    .getGamesInWeek(weekNum)
    .forEach(attachGameElement);
}
...
function attachGameElement(game) {
  ...
  awayTeamSelector.on("selectr.change", function (option) {
    game.awayTeam = getTeamByAbbreviation(
      scheduleService.file.schedule.teamData,
      option.value,
    );
    changeTeamLogo(awayTeamLogo, game.awayTeam.logoPath);
  });
  ...
  homeTeamSelector.on("selectr.change", function (option) {
    game.homeTeam = getTeamByAbbreviation(
      scheduleService.file.schedule.teamData,
      option.value,
    );
    changeTeamLogo(homeTeamLogo, game.homeTeam.logoPath);
...
function getDayOfWeekByAbbreviation(abbreviation) {
  return dayOfWeekData.days.find((day) => {
    return day.abbreviation === abbreviation;
  });
}
